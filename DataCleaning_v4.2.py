...
"""
Note!!!
This script is for leak data only
"""
...
tr_meter = pd.read_feather(base_data_dir / 'leak_data.feather', use_threads=True)
...
meter0 = mt_gps.get_group(0)


# for training data
meter0_cleaned_mat, meter0_mask = clean_meter_reading2(meter0)
...
meter1_cleaned_mat2, _ = clean_temporal_bands(meter1_cleaned_mat, bmeta, thresh=0.1)
...
meter3_cleaned_mat3, _ = clean_isolated_bands(meter3_cleaned_mat2,)
...
cleaned_meters.reset_index(drop=True).to_feather(processed_data_dir/'cleaned_leak_meter_log_1217.feather')
...
# cleaned_meters['year'] = cleaned_meters.index.year
# cleaned_meters_ygps = cleaned_meters.groupby('year')
cleaned_meters_ygps = cleaned_meters.groupby(lambda x: x.year)
new_year_meters = []
for year, gp in cleaned_meters_ygps:
    print(type(year))
    year_meters = gp.copy()
    year_meters['time_delta'] = (gp.index.get_level_values(level='timestamp') - pd.to_datetime(str(year))).total_seconds() // 3600
    assert year_meters['time_delta'].min() == 0
    year_meters.reset_index(inplace=True)
    bad_zeros = find_bad_zeros(year_meters)

    bad_bid1099 = find_bad_building1099(year_meters)

    all_bad_zeros = bad_zeros.union(bad_bid1099)
    year_meters = year_meters.drop(year_meters.index[bad_zeros])
    year_meters.drop(columns=['time_delta'], inplace=True)
    year_meters = year_meters.reset_index(drop=True)
    new_year_meters.append(year_meters)

cleaned_meters2 = pd.concat(new_year_meters, axis=0)

assert cleaned_meters2.meter_reading.isna().sum() == 0
# todo add checks before saving

raise ValueError('To stop running')
cleaned_meters2.reset_index(drop=True).to_feather(processed_data_dir/'cleaned_leak_meter_log_1219.feather')
